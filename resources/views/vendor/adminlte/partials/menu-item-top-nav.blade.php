{{-- @if(empty($item['field_search']))
    @if (isset($item['search']) && $item['search'])
        <form action="{{ $item['href'] }}" method="{{ $item['method'] }}" class="form-inline ml-2 mr-2">
            <div class="input-group">
                <input class="form-control form-control-navbar" value="{!! request()->get('search') !!}" type="search" name="search"
                       placeholder="{{ __($item['text']) }}" aria-label="{{ $item['aria-label'] ?? $item['text'] }}">
